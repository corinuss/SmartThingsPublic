 ...
 *
 *  Modified version of Z-Wave Garage Door Opener, by SmartThings.
 *  Author: Eric Will
 *  Date: 2018-01-21
 */
metadata {
	definition (name: "Iris Z-Wave Garage Door Opener", namespace: "corinuss", author: "Eric Will") {
		capability "Actuator"
		...
		capability "Refresh"
		capability "Sensor"
		capability "Lock" // For IFTTT support.  Maps to Door Control functionality.

		fingerprint mfr:"014F", prod:"4744", model:"3530", deviceJoinName: "Iris Garage Door Opener"
	}
...
def zwaveEvent(BarrierOperatorReport cmd) {
	...
	switch (cmd.barrierState) {
		...
		case BarrierOperatorReport.BARRIER_STATE_UNKNOWN_POSITION_STOPPED:
			...
			map.value = "unknown"
			result << createEvent(name: "lock", value: "unknown", displayed: false)
			break
